...
function ProductCard({
  p, cartQty, onInc, onDec,
}: {
  p: Producto; cartQty: number; onInc: () => void; onDec: () => void;
}) {
    ...
    <div className="bg-white rounded-2xl border border-neutral-200 flex flex-col overflow-hidden">
      {p.cover_image_url ? (
        <div className="relative w-full aspect-[4/3] bg-neutral-100">
          <Image
            src={p.cover_image_url} alt={p.name} fill
...
export function CatalogoB2BClient({
  ...
  const [cart, dispatch] = useReducer(cartReducer, undefined, (): CartItem[] => {
    ...
    try {
      ...
      return products
        ...
        .map((p) => ({
          id: p.id, name: p.name, unit_label: p.unit_label,
          bolsas_caja: p.bolsas_caja, precio: p.precio!, qty: qtys[p.id],
        }));
    } catch { return []; }
  });
  ...
  const lineas = Array.from(new Set(products.map((p) => p.linea))).filter(Boolean);

  const filtered = products.filter((p) => {
    const matchLinea = activeLinea === "todas" || p.linea === activeLinea;
    const matchText  = search.trim() === "" ||
      p.name.toLowerCase().includes(search.toLowerCase());
    return matchLinea && matchText;
  });

  const totalQty  = cart.reduce((s, i) => s + i.qty, 0);
  const totalCiva = cart.reduce((s, i) => s + i.precio.final_civa * i.qty, 0);
    ...
    <>
      ...
      <div className="overflow-x-auto -mx-4 px-4 md:mx-0 md:px-0 mb-6">
        <div className="flex items-center gap-2 w-max md:w-auto md:flex-wrap">
          <button onClick={() => setActiveLinea("todas")}
            className={`px-3.5 py-1.5 rounded-full text-sm font-medium transition-colors ${activeLinea === "todas" ? "bg-tierra-700 text-white" : "bg-white border border-neutral-200 text-neutral-600 hover:bg-neutral-50"}`}>
            Todas
          </button>
          {lineas.map((l) => (
            <button key={l} onClick={() => setActiveLinea(l)}
              className={`px-3.5 py-1.5 rounded-full text-sm font-medium transition-colors ${activeLinea === l ? "bg-tierra-700 text-white" : "bg-white border border-neutral-200 text-neutral-600 hover:bg-neutral-50"}`}>
              {l}
            </button>
          ))}
        </div>
      ...
      {filtered.length === 0 ? (
        <p className="text-sm text-neutral-400 text-center py-16">No hay productos que coincidan.</p>
      ) : (
        <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 ${totalQty > 0 ? "pb-48" : ""}`}>
          {filtered.map((p) => {
            ...
            const minQty   = p.min_quantity_b2b ?? 1;
            return (
              <ProductCard key={p.id} p={p} cartQty={cartItem?.qty ?? 0}
