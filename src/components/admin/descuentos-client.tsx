...
const inputCls = "px-3 py-2 text-sm border border-neutral-400 rounded-lg focus:outline-none focus:ring-[3px] focus:ring-brand-500/30 focus:border-brand-700 w-full";
      ...
      <tr className="bg-neutral-50">
        <td colSpan={5} className="px-5 py-4">
          <form onSubmit={handleGuardar} className="flex gap-3 items-end flex-wrap">
            ...
            <div>
              <label className="block text-[13px] font-medium text-neutral-800 mb-1.5">Mín. cajas</label>
              <input type="number" name="min_cajas" min={1} defaultValue={tier.min_cajas}
                className={`${inputCls} w-28`} disabled={isPending} />
            </div>
            <div>
              <label className="block text-[13px] font-medium text-neutral-800 mb-1.5">Descuento %</label>
            ...
            <div className="flex-1 min-w-[200px]">
              <label className="block text-[13px] font-medium text-neutral-800 mb-1.5">Etiqueta</label>
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={() => setEditando(false)}
                className="px-3 py-2 text-xs text-neutral-600 hover:text-neutral-700">
                Cancelar
              </button>
            </div>
          </form>
  ...
  return (
    <tr className={`hover:bg-neutral-50 transition-colors ${!tier.activo ? "opacity-50" : ""}`}>
      ...
      <td className="px-5 py-3">
        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${tier.activo ? "bg-success-bg text-success" : "bg-neutral-100 text-neutral-600"}`}>
...
function NuevoTierForm() {
  ...
  if (!open) {
    return (
      <button onClick={() => setOpen(true)}
        className="px-4 py-2 text-sm font-medium rounded-lg bg-tierra-700 text-white hover:bg-tierra-800 transition-colors">
        + Agregar escalón
      </button>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex gap-3 items-end flex-wrap p-4 bg-neutral-50 rounded-xl border border-neutral-200">
      ...
      <div className="flex-1 min-w-[200px]">
        <label className="block text-[13px] font-medium text-neutral-800 mb-1.5">Etiqueta (visible al vendedor)</label>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-neutral-100 text-left">
              <th className="px-5 py-3 text-xs font-semibold text-neutral-600">Cajas mínimas</th>
              <th className="px-5 py-3 text-xs font-semibold text-neutral-600">Descuento</th>
              <th className="px-5 py-3 text-xs font-semibold text-neutral-600">Etiqueta</th>
          ...
          <tbody className="divide-y divide-neutral-50">
            {tiers.length === 0 && (
              <tr>
                <td colSpan={5} className="px-5 py-10 text-center text-xs text-neutral-600">
