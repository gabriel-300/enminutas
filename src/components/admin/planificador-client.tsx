...
export function PlanificadorClient({
  ...
  const inputCls = "px-3 py-2 text-sm border border-neutral-400 rounded-lg focus:outline-none focus:ring-[3px] focus:ring-brand-500/30 focus:border-brand-700 bg-white";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-neutral-200 shadow-sm p-5">
        <p className="text-xs font-semibold text-neutral-600 mb-4">Configuración del turno</p>
        <div className="flex items-end gap-6 flex-wrap">
          <div>
            <label className="block text-[13px] font-medium text-neutral-800 mb-1.5">Personas disponibles</label>
          ...
          <div>
            <label className="block text-[13px] font-medium text-neutral-800 mb-1.5">Horas de turno</label>
          ...
          <div className="ml-auto text-right">
            <p className="text-xs text-neutral-600 mb-0.5">Tiempo disponible</p>
            <p className="text-2xl font-semibold font-display text-neutral-900">{fmtMin(disponibleMin)}</p>
            <p className="text-xs text-neutral-600">{personas} persona{personas !== 1 ? "s" : ""} × {horas}h</p>
          </div>
        </div>
      </div>
      ...
      {noHayNada ? (
        <div className="bg-white rounded-xl border border-neutral-200 shadow-sm p-12 text-center">
          <p className="text-neutral-600 text-sm">No hay productos que necesiten producción ahora.</p>
          <p className="text-xs text-neutral-500 mt-1">Todo el stock está en nivel óptimo.</p>
        </div>
      ) : (
        <>
          {/* Capacidad */}
          <div className="bg-white rounded-xl border border-neutral-200 shadow-sm p-5 space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold text-neutral-600">Uso del turno</p>
            ...
            <div className="bg-white rounded-xl border border-neutral-200 shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-neutral-100 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-neutral-600 mt-0.5">{enPlan.length} producto{enPlan.length !== 1 ? "s" : ""} · {fmtMin(totalMin)}</p>
                </div>
              </div>
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-neutral-100 text-left">
                    <th className="px-5 py-3 text-xs font-semibold text-neutral-600 w-8">#</th>
                    <th className="px-5 py-3 text-xs font-semibold text-neutral-600">Producto</th>
                    <th className="px-5 py-3 text-xs font-semibold text-neutral-600 text-center">Cajas a producir</th>
                    <th className="px-5 py-3 text-xs font-semibold text-neutral-600 text-center">Stock actual</th>
                    <th className="px-5 py-3 text-xs font-semibold text-neutral-600 text-right">Tiempo est.</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-neutral-50">
                  {enPlan.map((item, idx) => (
                    <tr key={item.id} className={item.urgente ? "bg-warning-bg/20" : ""}>
                      <td className="px-5 py-3 text-xs font-semibold text-neutral-500 tabular-nums">{idx + 1}</td>
                      <td className="px-5 py-3">
                        <p className="font-medium text-neutral-900">{item.name}</p>
                        <p className="text-xs text-neutral-600 font-mono">{item.sku}</p>
                        {item.demanda > 0 && (
                      ...
                      <td className="px-5 py-3 text-center text-sm tabular-nums">
                        <span className={item.stock === 0 ? "text-danger font-semibold" : item.urgente ? "text-warning font-semibold" : "text-neutral-600"}>
                          {item.stock}
                        </span>
                        {item.minimo > 0 && <span className="text-neutral-500 text-xs"> / {item.minimo}</span>}
                      </td>
                ...
                <tfoot>
                  <tr className="border-t-2 border-neutral-200 bg-neutral-50">
                    <td colSpan={4} className="px-5 py-3 text-xs font-semibold text-neutral-600 text-right">Total</td>
                ...
                <tbody className="divide-y divide-neutral-50">
                  {fuera.map((item) => (
                    <tr key={item.id} className="opacity-60">
                      ...
                      <td className="px-5 py-3 text-right text-sm text-neutral-600 tabular-nums">
          ...
          {sinReceta.length > 0 && (
            <div className="bg-neutral-50 rounded-xl border border-neutral-200 p-5">
              <p className="text-xs font-semibold text-neutral-600 mb-2">Sin receta cargada</p>
