...
function saldoColor(saldo: number, limite: number) {
  if (saldo <= 0) return { text: "text-success", bg: "bg-success-bg" };
  if (limite > 0 && saldo >= limite * 0.9) return { text: "text-danger", bg: "bg-danger-bg" };
  if (limite > 0 && saldo >= limite * 0.7) return { text: "text-warning", bg: "bg-warning-bg" };
  ...
  return (
    <div className="p-4 md:px-10 md:py-8 md:pb-16">
      <div className="mb-6">
        ...
        <p className="text-sm text-neutral-600 mt-1">Saldos y límites de crédito por cliente B2B</p>
      </div>

      {/* KPIs */}
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="bg-white rounded-xl border border-neutral-200 shadow-sm p-5">
          <p className="text-xs text-neutral-600 mb-1">Deuda total</p>
          <p className="text-2xl font-bold text-neutral-900 tabular-nums">{fmt(totalDeuda)}</p>
        </div>
        <div className="bg-white rounded-xl border border-neutral-200 shadow-sm p-5">
          <p className="text-xs text-neutral-600 mb-1 flex items-center gap-1">
        ...
        <div className="bg-white rounded-xl border border-neutral-200 shadow-sm p-5">
          <p className="text-xs text-neutral-600 mb-1 flex items-center gap-1">
            <AlertCircle className="size-3 text-danger" /> En riesgo
          </p>
          <p className="text-2xl font-bold text-danger">{enRiesgo}</p>
          <p className="text-xs text-neutral-600 mt-0.5">≥90% del límite</p>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead className="border-b border-neutral-100">
            <tr>
              ...
              <th className="px-5 py-3 text-right text-xs font-semibold text-neutral-600">Saldo</th>
              <th className="px-5 py-3 text-right text-xs font-semibold text-neutral-600">Límite</th>
              <th className="px-5 py-3 text-right text-xs font-semibold text-neutral-600">Uso</th>
              ...
              return (
                <tr key={f.profile_id} className="hover:bg-neutral-50 transition-colors">
                  ...
                  <td className="px-5 py-3.5 text-right text-neutral-600 tabular-nums">
                    {f.limite > 0 ? fmt(f.limite) : <span className="text-neutral-500">—</span>}
                  </td>
                  <td className="px-5 py-3.5 text-right">
                    {uso !== null ? (
                      <div className="flex items-center justify-end gap-2">
                        <div className="w-16 h-1.5 bg-neutral-100 rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full ${uso >= 90 ? "bg-danger-solid" : uso >= 70 ? "bg-warning-solid" : "bg-success-solid"}`}
                            style={{ width: `${uso}%` }}
                          />
                        </div>
                        <span className="text-xs text-neutral-600 tabular-nums w-8 text-right">{uso}%</span>
                      </div>
                    ) : (
                      <span className="text-neutral-500 text-xs">sin límite</span>
                    )}
                  </td>
                  <td className="px-5 py-3.5 text-right">
                    <Link
