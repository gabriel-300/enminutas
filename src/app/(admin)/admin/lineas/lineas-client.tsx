...
import { crearLinea, eliminarLinea } from "./actions";
...
export function LineasClient({ lineas }: { lineas: Linea[] }) {
  ...
  function handleEliminar(id: number, nom: string) {
    if (!confirm(`¿Eliminar la línea "${nom}"?`)) return;
    setError(null);
    startTransition(async () => {
      const res = await eliminarLinea(id);
      ...
      <div className="bg-white rounded-2xl border border-neutral-200 divide-y divide-neutral-100">
        ...
        {lineas.map(l => (
          <div key={l.id} className="flex items-center gap-3 px-5 py-3">
            ...
            <span className="text-xs text-neutral-400 mr-3">{l.total} producto{l.total !== 1 ? "s" : ""}</span>
            <button
              onClick={() => handleEliminar(l.id, l.nombre)}
