    ...
    <div className="p-4 md:px-10 md:py-8 md:pb-16">
      <div className="mb-5 md:mb-6">
        <div className="flex items-start justify-between gap-3 mb-1">
          ...
          <div className="flex gap-2 shrink-0">
            <Link
              href="/repartidor"
              className="flex items-center gap-1 px-2.5 py-2 rounded-lg border border-brand-700 text-sm font-medium text-brand-700 hover:bg-brand-700 hover:text-white transition-colors"
              title="App mobile para repartidores"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="size-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z" />
              </svg>
              <span className="hidden sm:inline">App repartidor</span>
            </Link>
            <Link
              href="/admin/distribucion/historial"
              className="flex items-center gap-1 px-2.5 py-2 rounded-lg border border-n-btn text-sm font-medium text-neutral-800 hover:bg-neutral-50 transition-colors"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="size-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            {lista.length > 0 && (
              <>
                <Link
                  href="/admin/distribucion/ordenar-ruta"
                  className="flex items-center gap-1 px-2.5 py-2 rounded-lg border border-n-btn text-sm font-medium text-neutral-800 hover:bg-neutral-50 transition-colors"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="size-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 12h16M4 18h7" />
                  </svg>
                  <span className="hidden sm:inline">Ordenar ruta</span>
                </Link>
                <Link
        ...
        <p className="text-sm text-neutral-600">
          ...
          {zonaNombre && <span className="ml-2 text-neutral-600">— {zonaNombre}</span>}
        </p>
      ...
      {lista.length === 0 ? (
        <div className="bg-white rounded-xl border border-neutral-200 shadow-sm p-12 text-center">
          <p className="text-neutral-600 text-sm">No hay pedidos despachados pendientes de entrega.</p>
        </div>
      ) : filteredZoneNames.length === 0 ? (
        <div className="bg-white rounded-xl border border-neutral-200 shadow-sm p-12 text-center">
          <p className="text-neutral-600 text-sm">No hay pedidos para la zona seleccionada.</p>
        </div>
      ) : (
              ...
              <div className="space-y-3">
                {byZone[zoneName].map((order) => {
                  ...
                  return (
                    <div
                      key={order.id}
                      className={`bg-white rounded-xl border p-4 md:p-5 ${dias !== null && dias > 3 ? "border-danger-border" : "border-neutral-200"}`}
                    >
                      ...
                      {phone && (
                        <div className="flex items-center gap-3 mb-1">
                          <a href={`tel:${phoneClean}`} className="text-xs text-neutral-600 hover:text-neutral-800">
                            {phone}
                          </a>
                          <a
                      ...
                      {order.despacho_info && (
                        <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-xs text-neutral-600 mb-2">
        ...
        <div className="mt-10">
          ...
          <div className="space-y-2">
            {listaHoy.map((order: any) => (
              <div key={order.id} className="bg-white rounded-xl border border-neutral-100 p-4 opacity-75">
                <div className="flex items-center gap-3">
                  <span className="text-success text-sm">✓</span>
                  <span className="font-mono text-xs text-neutral-600">{order.order_number}</span>
                  <span className="text-sm font-medium text-neutral-700">{order.customer?.full_name ?? "—"}</span>
                  {order.customer?.zona && (
                    <span className="text-xs text-neutral-600">· {order.customer.zona.name}</span>
                  )}
                </div>
