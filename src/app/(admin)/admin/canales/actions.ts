...
export async function crearCanal(formData: FormData) {
  ...
  const margen_venta_directa  = Number(formData.get("margen_venta_directa")) / 100 || 0;
  ...
  const { error } = await (supabase as any).from("canales").insert({
    ...
    margen_std, margen_premium, margen_venta_directa, markup_pvp,
  });
...
export async function actualizarCanal(id: string, formData: FormData) {
  ...
  const { error } = await (supabase as any)
    .from("canales")
    .update({ nombre, descuento_pct, sort_order, activo, margen_std, margen_premium, margen_venta_directa, markup_pvp })
    .eq("id", id);
  if (error) throw new Error(error.message);
